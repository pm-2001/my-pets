#!/usr/bin/env node
// CLI entry point: `npx desktop-pet` or, after `npm i -g desktop-pet`, just
// `desktop-pet`. Launches the prebuilt app under Electron. The app is shipped
...
if (process.platform !== 'darwin') {
  console.error(
    'desktop-pet currently runs on macOS only.\n' +
...
try {
  ...
  console.error('Could not find the electron runtime. Try reinstalling: npm i -g desktop-pet')
  process.exit(1)
}
...
child.on('error', (err) => {
  console.error('Failed to launch desktop-pet:', err.message)
  process.exit(1)
})
